"""Game logic and event emission for Mining Ways Hold & Spin game.

Features:
- 5 reels x 6 rows ways game (7,776 ways)
...
class GameState(GameStateOverride):
    ...
    def run_hold_and_spin(self, is_freespin=False) -> None:
        ...
        while self.holdnspin_respins > 0:
            ...
            # If in free spins, grow multiplier each round
            if is_freespin:
            ...
            # Check for wincap during Hold & Spin
        ...
        # Update win manager (add H&S win to spin_win and running total)
        # Note: update_gametype_wins is NOT called here - the caller handles it
        # so that all wins (ways + H&S) are tracked together
    ...
    def check_freespin_entry(self, scatter_key: str = "scatter") -> bool:
        ...
        # (Free spins would interfere with the H&S-only requirement)
        ...
        if not conditions.get("force_freegame", False):
            for reel_idx, reel in enumerate(self.board):
                for row_idx, sym in enumerate(reel):
                    if sym.name == "S":
                        # Replace scatter with a random low-pay symbol
    ...
    def draw_board(self, emit_event: bool = True, trigger_symbol: str = "scatter") -> None:
        """Override draw_board to also handle Hold & Spin forced distributions.
        ...
        if conditions.get("force_holdnspin", False) and self.gametype == self.config.basegame_type:
            ...
            # Normal basegame: ensure no scatter trigger for non-FS criteria
            self.create_board_reelstrips()
